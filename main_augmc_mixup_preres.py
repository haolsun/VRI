...
parser.add_argument('--corruption_prob', type=float, default=0.6, help='label noise')
parser.add_argument('--corruption_type', '-ctype', type=str, default='inst', help='Type of corruption ("unif" or "flip" or "inst").')
...
def build_dataset(root, args):
    ...
    if args.dataset == 'cifar10':
        ...
        train_data = CIFAR10(device,
                ...
                corruption_type=args.corruption_type, transform=strong_transform, download=True, seed=args.seed, strong_t=strong_transform, normalize=normalize)
...
def train(train_loader, train_meta_loader, model, vnet, vnet_prior, optimizer_model, optimizer_vnet, optimizer_vnet_prior, epoch):
    ...
    for batch_idx, (inputs, targets, path) in enumerate(tqdm(train_loader, ncols=0)):
        ...
        # v_lambda, kl_loss = vnet(feat.detach(), targets)
...
def build_training(args):
    ...
    vnet_prior = vri_prior(512, 1024, num_classes).cuda()
...
def main():
    ...
    # vnet_prior = vri_prior(512, 1024, num_classes).cuda()
    #
    # optimizer_wp = torch.optim.SGD(model.params(), 0.02, momentum=args.momentum, weight_decay=args.weight_decay)
    # optimizer_model = torch.optim.SGD(model.params(), 0.02, momentum=args.momentum, weight_decay=args.weight_decay)
    # # optimizer_model = torch.optim.Adam(model.params(), lr=3e-4)
    # sch_lr = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_model, T_max=5, eta_min=1e-4)
    # optimizer_vnet = torch.optim.Adam(vnet.params(), 3e-4, weight_decay=args.weight_decay)
    # optimizer_vnet_prior = torch.optim.Adam(vnet_prior.params(), 3e-4, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.epochs):
        # adjust_learning_rate(optimizer_model, T_max=args.epochs, eta_min=0.0001, last_epoch=-1)
...
if __name__ == '__main__':
    ...
    txt_name = args.dataset + '_' + args.corruption_type + '_' + str(args.corruption_prob) + '_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    print(txt_name)
